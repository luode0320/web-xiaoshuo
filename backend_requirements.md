...
- **处理流程**:
  ...
  4. 计算文件hash值（使用crypto/sha256库）
  5. 验证文件格式和大小（使用filetype库）
  6. 检查文件hash是否已存在
  7. 保存文件到指定目录
  8. 提取小说元数据（包括主角名称，使用golang-epub库处理EPUB文件）
  9. 计算并存储字数统计（使用golang-runewidth库）
  10. 创建小说记录（状态为审核中，关联上传用户ID）
- **响应**: 小说信息
...
- **处理流程**:
  1. 验证用户认证
  2. 验证用户权限（只能编辑自己的评论）
  3. 检查评论创建时间是否在30分钟编辑窗口期内（使用time库计算时间差）
  4. 使用bluemonday库过滤内容中的恶意代码
  5. 更新评论内容
  6. 更新评论更新时间
...
- **处理流程**:
  1. 验证用户认证
  2. 验证用户权限（只能编辑自己的评分）
  3. 检查评分创建时间是否在30分钟编辑窗口期内（使用time库计算时间差）
  4. 更新评分内容
  5. 更新评分更新时间
