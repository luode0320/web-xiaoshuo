...
### 1. 用户管理功能
- 用户注册：使用邮箱注册，只需符合邮箱格式即可完成注册，无需邮箱验证
- 用户登录：使用邮箱和密码进行登录，无需验证码
- 个人中心：展示用户基本信息和操作历史

### 2. 小说管理功能
- 小说上传：支持txt、epub格式小说文件上传，文件大小限制最大20MB，包含文件hash验证防止重复上传，上传失败时提示错误信息。上传成功后等待后端完成文件解析（使用第三方库如go-epub、golang-runewidth等进行字数统计、章节提取等）后再刷新列表
...
### 4. 用户关联功能
- 用户评论：用户对小说章节的评论通过用户ID关联，便于管理和展示
- 用户评分：用户对小说的评分通过用户ID关联，便于管理和展示
