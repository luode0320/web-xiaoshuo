...
- 个性化设置：字体大小（范围[0-100px]）、背景颜色（白、黑、灰三种选项）等阅读界面定制
...
### 4. 社交互动功能
- 评论系统：用户可以对小说章节进行评论，每次小说的每一章的最后一页可以对小说的这一章节进行评论。前端要在每一章的最后可以提供评论的查看和用户提交评论，和对评论进行评论，评论用一个弹窗分页展示
- 评分系统：用户可以对小说进行评分，同时需要提供评分说明（用于对整个小说的评论展示，最多250个字符，非必填），评分中的评论支持点赞（同一个IP限制点赞一次，点赞状态在用户本地保存）
...
- 评论弹窗（点击评论可打开评论列表弹窗，居中显示，可右上角关闭，支持滚动分页，默认每页显示5条评论，支持按点赞最多和最新排序，点赞最多为默认排序，评论数据来自小说评分评论而非章节评论）
- 相关推荐（推荐拥有相同分类或者关键字的小说，按照阅读量排序的最多3本小说作为推荐，排除本地存储的已读小说，向下滚动即可看到）
